 ...
 * It escapes all Names, ExtendedNames and non-subtext Strings with \mathrm{}
 * If you don't know which one to use, use this one
 */
public class MathJaxExpressionPrinter2 extends FdlTraverserImplementation {

  private final StringBuilder sb;

  public MathJaxExpressionPrinter2() {
  ...
  public void traverse(ASTExtendedName node) {
    ...
    if (node.getIdentifier().isPresentName()) {
      sb.append("\\mathrm{").append(node.getIdentifier().getName()).append("}");
    } else {
      sb.append("\\");
